...
app = Celery('gosint.port_api', broker=broker, backend=backend, )
...
def port_api_fofa(email, key, file):
    FOFA_EMAIL = email
    FOFA_KEY = key
    ...
    if task.result() and file:
        with open(file) as f:
            ...
            for i in f.readlines():
                i = i.strip()
                search_keywords = "ip="+str(i)
                print(f"fofa查询语法: {search_keywords}")
                search_keyword = base64.b64encode(search_keywords.encode()).decode()
                url = f"https://fofa.so/api/v1/search/all?email={FOFA_EMAIL}&key={FOFA_KEY}&qbase64={search_keyword}&size=10000&fields=host,ip,domain,port,server"
                # print(url)
                loop = asyncio.get_event_loop()
...
def run(target):
    ...
    result = []
    try:
        res = port_api_fofa(email, key, target_file)
        for jsonhtml in res:
            htmlsize = jsonhtml['size']
            htmlresults = jsonhtml['results']
            for i in range(0, htmlsize):
                dic = {}
                dic["host"], dic["ip"], domain, dic["port"], dic["server"] = htmlresults[i]
                result.append(dic)
    ...
    return {'tool': 'port_api', 'result': result}
